...
import type { KnowledgeNode, NodeType, NodeStatus, User, AuditLog } from '../types';
...
export default function NodeDetailPanel({ node, currentUser, auditLogs, onAction, isLoading }: NodeDetailPanelProps) {
  const [activeForm, setActiveForm] = useState<'NONE' | 'CONFIRM' | 'SUPERSEDE' | 'EXPIRE' | 'HOLD'>('NONE');
  ...
  if (!node) {
    return (
      <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-2xl h-full flex flex-col items-center justify-center text-slate-500 text-xs">
        ...
        <span>Select any node in the cascade visualization to view details, enforce state changes, or complete a human review.</span>
      </div>
    );
  }
  ...
  const getTypeStyle = (type: NodeType) => {
    switch (type) {
      case 'CONSTRAINT': return 'bg-purple-500/10 text-purple-400 border border-purple-500/20';
      case 'DECISION': return 'bg-blue-500/10 text-blue-400 border border-blue-500/20';
      case 'ANTI_PATTERN': return 'bg-amber-500/10 text-amber-400 border border-amber-500/20';
  ...
  const getStatusBadge = (status: NodeStatus) => {
    switch (status) {
      case 'ACTIVE': return 'bg-emerald-500 text-slate-950 font-extrabold shadow-[0_0_8px_rgba(16,185,129,0.3)]';
      case 'REVIEW_REQUIRED': return 'bg-amber-500 text-slate-950 font-extrabold shadow-[0_0_8px_rgba(245,158,11,0.3)] animate-pulse';
      case 'SUPERSEDED': return 'bg-slate-800 text-slate-400 border border-slate-700';
      case 'EXPIRED': return 'bg-rose-500/20 text-rose-400 border border-rose-500/30';
      case 'LEGAL_HOLD': return 'bg-indigo-500 text-white font-extrabold shadow-[0_0_8px_rgba(99,102,241,0.3)]';
    }
  };

  const initSupersedeForm = () => {
    setNewTitle(`${node.title} v3`);
    setNewContent(node.content.replace(/v2 \(2024\)/g, 'v3 (2026)').replace(/3 HOURS/g, '1 HOUR'));
    setNewType(node.type);
    setNewImportance(node.importance);
    setValidUntil('');
    setComment('Upgrading sepsis protocol to v3 clinical standard (lactate measurement within 1 hour).');
  ...
  return (
    <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-2xl h-full flex flex-col overflow-y-auto max-h-[750px] scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
        ...
        <div className="flex justify-between items-start gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              <span className="text-[10px] font-mono text-slate-500 font-bold">{node.id}</span>
            ...
            <h3 className="text-base font-bold text-slate-100 leading-snug">{node.title}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="mb-4 text-left">
        <h4 className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mb-1">Content / Formula</h4>
      ...
      <div className="grid grid-cols-2 gap-4 mb-5 p-3.5 bg-slate-950/20 rounded-xl border border-slate-800 text-xs text-left">
        <div>
          <span className="text-slate-500 block mb-0.5 text-[10px]">Department</span>
          <span className="font-semibold text-slate-355 capitalize">{node.department || 'General Hospital'}</span>
        </div>
        <div>
          <span className="text-slate-500 block mb-0.5 text-[10px]">Importance Weighted</span>
          <span className="font-semibold text-slate-355">{node.importance.toFixed(2)}</span>
        </div>
        <div>
          <span className="text-slate-500 block mb-0.5 text-[10px]">Created By</span>
          <span className="font-semibold text-slate-355">{node.created_by}</span>
        </div>
        <div>
          <span className="text-slate-500 block mb-0.5 text-[10px]">Valid Until</span>
          <span className="font-semibold text-slate-355">
            {node.valid_until ? new Date(node.valid_until).toLocaleDateString() : 'Never Expiable'}
          </span>
        </div>
      </div>
      ...
      {!isOldNodeSuperseded && activeForm === 'NONE' && (
        <div className="space-y-3 mb-6">
          <h4 className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mb-2 text-left">Governance Actions</h4>
          ...
          {node.id === 'N-M08' && node.status === 'ACTIVE' && (
            ...
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.228 9H18.01" />
              </svg>
          ...
          <div className="flex gap-2 text-left">
            {node.status !== 'LEGAL_HOLD' ? (
              <button
                ...
                title={!isAdmin ? "Only Administrator can enforce Legal Hold" : ""}
              ...
              <button
                ...
                disabled={isLoading || !isAdmin}
                title={!isAdmin ? "Only Administrator can release Legal Hold" : ""}
                className={`w-full py-2 px-4 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                  isAdmin 
        ...
        <div className="bg-slate-950/30 border border-slate-800 rounded-xl p-4 mb-6 text-left">
          <div className="flex justify-between items-center mb-3">
            <h5 className="text-xs font-bold text-slate-200">
              ...
              {activeForm === 'SUPERSEDE' && 'Supersede Node (Create New Version)'}
          ...
          {activeForm === 'CONFIRM' && (
            <form onSubmit={handleConfirmSubmit} className="space-y-3">
              <div>
                <label className="text-[9px] uppercase font-bold text-slate-500 block mb-1">Reasoning / Comments</label>
                <textarea
                  value={comment}
          ...
          {activeForm === 'EXPIRE' && (
            <form onSubmit={handleExpireSubmit} className="space-y-3">
              <div>
                <label className="text-[9px] uppercase font-bold text-slate-500 block mb-1">Reason for Expiration</label>
                <textarea
                  value={comment}
                  onChange={e => setComment(e.target.value)}
          ...
          {activeForm === 'SUPERSEDE' && (
            <form onSubmit={handleSupersedeSubmit} className="space-y-3.5">
              <div>
                <label className="text-[9px] uppercase font-bold text-slate-500 block mb-1">New Version Title</label>
                <input
                  type="text"
                  value={newTitle}
              ...
              <div>
                <label className="text-[9px] uppercase font-bold text-slate-500 block mb-1">New Content / Parameters</label>
                <textarea
                  value={newContent}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[9px] uppercase font-bold text-slate-500 block mb-1">Node Type</label>
                  <select
                    value={newType}
                ...
                <div>
                  <label className="text-[9px] uppercase font-bold text-slate-500 block mb-1">Importance (0.0 - 1.0)</label>
                  <input
                    type="number"
                    step="0.05"
                    min="0.1"
              ...
              <div>
                <label className="text-[9px] uppercase font-bold text-slate-500 block mb-1">Expiry Date (Optional)</label>
                <input
                  type="date"
                  value={validUntil}
              ...
              <div>
                <label className="text-[9px] uppercase font-bold text-slate-500 block mb-1">Supersession Justification</label>
                <textarea
                  value={comment}
                  onChange={e => setComment(e.target.value)}
      ...
      <div className="flex-1 mt-2 text-left">
        <h4 className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mb-3">Audit Trail / Node History</h4>
        
        {auditLogs.length === 0 ? (
          <p className="text-[10px] text-slate-650 italic">No logs recorded for this node.</p>
        ) : (
            ...
            {auditLogs.map((log, idx) => (
              <div key={log.id || idx} className="relative text-left">
                ...
                <div className="flex items-center gap-1.5 text-[9px] text-slate-500 mt-1">
                  <span>Actor: <span className="font-semibold text-slate-400">{log.actor_id}</span></span>
                  {log.old_value && (
                    <span>• Status: <span className="font-semibold">{log.old_value}</span> → <span className="font-semibold text-slate-400">{log.new_value}</span></span>
                  )}
                </div>
              </div>
            ))}
