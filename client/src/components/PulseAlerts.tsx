...
export default function PulseAlerts({ alerts, onMarkRead, onMarkAllRead, onSelectNode }: PulseAlertsProps) {
    ...
    <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-2xl flex flex-col h-full">
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Targeted notifications routed by role</p>
        </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start gap-2">
                    <h3 className={`flex-1 min-w-0 text-xs font-bold truncate leading-tight ${alert.is_read ? 'text-slate-400' : 'text-slate-100'}`}>
                      {alert.title}
                    </h3>
                    <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase shrink-0 ${getSeverityStyles(alert.severity)}`}>
                      {alert.severity}
                    </span>
                  </div>
                  <p className={`text-[11px] mt-1 leading-normal ${alert.is_read ? 'text-slate-500' : 'text-slate-300'}`}>
                    {alert.body}
                  </p>
