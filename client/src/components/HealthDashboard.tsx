...
export default function HealthDashboard({
  ...
  const getBarColorClass = (val: number) => {
    ...
    if (val >= 0.7) return "bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.3)]";
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Real-time vitals representing governance quality
          </p>
        </div>
          ...
          {dimensions.map((dim, idx) => (
            <div key={idx} className="space-y-1.5">
              <div className="flex justify-between items-center text-xs">
                <span className="font-semibold text-slate-200" title={dim.desc}>
                  {dim.name}
                </span>
                <span
                  className={`font-bold tabular-nums px-1.5 py-0.5 rounded ${getScoreColor(dim.value)}`}
                >
                  {formatPct(dim.value)}
                </span>
              </div>
        ...
        <div className="mt-4 p-3 bg-amber-500/5 border border-amber-500/10 rounded-xl flex items-start gap-2.5">
          ...
          <div className="text-[11px] text-slate-300 leading-normal">
            <span className="font-semibold text-amber-400">
              Score recalculation is deferred.
            </span>{" "}
            Immediate scores are frozen during cascade changes to prevent visual
            alarm. Click &quot;Recompute&quot; above to calculate live stats.
          </div>
        </div>
      )}
    </div>
